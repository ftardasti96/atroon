const VIEW_PORT = "about-particle";
const VIEW_PORT_COLOR = 0x000000;
const IMAGE_URL = "image/STARS.png";
...
class ImageParticleSystem {
  constructor() {
    ...
    this.renderer = PIXI.autoDetectRenderer(
      ...
      window.innerHeight,
      {
        view: document.getElementById(VIEW_PORT),
        backgroundColor: VIEW_PORT_COLOR
      }
    );
  ...
  createParticles() {
    ...
    const imageScale = min(
      (window.innerWidth - PADDING * 2) / imageWidth,
      (window.innerHeight - PADDING * 2) / imageHeight
    );
  ...
  clearParticles() {
    pointSystem.container.removeChildren();
    pointSystem.points = [];
  }
  updateState() {
